...
const GlobalStyles = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .fade-up2 { animation: fadeUp .5s .12s ease both; }
    .fade-up3 { animation: fadeUp .5s .22s ease both; }
    .fade-up4 { animation: fadeUp .5s .32s ease both; }
    .fade-up5 { animation: fadeUp .5s .42s ease both; }
    ...
    .google-btn:hover { background:rgba(255,255,255,.1); border-color:rgba(255,255,255,.24); }
    .g-icon {
      width:22px; height:22px; border-radius:50%; background:#fff;
      display:flex; align-items:center; justify-content:center;
      font-weight:900; font-size:.78rem; color:#4285F4; flex-shrink:0;
    }
    ...
    .form-input {
      ...
      color:var(--text-primary); font-family:'DM Sans',sans-serif; font-size:.875rem;
    ...
    .role-pill {
      flex:1; padding:.6rem .4rem; border-radius:10px; border:1.5px solid var(--glass-border);
    ...
    .auth-btn {
      ...
      cursor:pointer; transition:all .2s; margin-top:.2rem;
      display:flex; align-items:center; justify-content:center; gap:.5rem;
      box-shadow:0 6px 22px rgba(37,99,235,.33);
    }
    ...
    .auth-switch a { color:#60a5fa; font-weight:700; cursor:pointer; }
    .auth-switch a:hover { text-decoration:underline; }

    /* demo hint */
    .demo-hint {
      margin-top:.9rem; padding:.7rem 1rem;
      background:rgba(244,185,66,.06); border:1px solid rgba(244,185,66,.15); border-radius:10px;
    }
    ...
    /* error box */
    .err-box {
      padding:.65rem 1rem; background:rgba(232,69,69,.12); border:1px solid rgba(232,69,69,.3);
      border-radius:10px; color:#f87272; font-size:.82rem; margin-bottom:.85rem;
    ...
    .stars-bg {
      ...
      background:
        radial-gradient(ellipse at 20% 60%,#1a1060 0%,transparent 50%),
        radial-gradient(ellipse at 80% 10%,#102050 0%,transparent 40%),
        var(--navy);
    }
    ...
    .nav {
      ...
      background:rgba(8,14,43,.92); backdrop-filter:blur(16px);
    ...
    .nav-av {
      ...
      border:2px solid rgba(244,185,66,.38);
    }
    ...
    .btn-icon {
      width:33px; height:33px; border-radius:8px; border:1px solid var(--glass-border);
    ...
    .hero {
      ...
      background:linear-gradient(135deg,#1a2860,#0d1840);
      border:1px solid var(--glass-border); margin-bottom:2rem; position:relative; overflow:hidden;
    }
    .hero::before {
      content:''; position:absolute; top:-40%; right:-8%;
      width:360px; height:360px; border-radius:50%;
      background:radial-gradient(circle,rgba(244,185,66,.09) 0,transparent 70%);
    }
    ...
    .sinp {
      width:100%; padding:.7rem 1rem .7rem 2.7rem; border-radius:12px;
      border:1.5px solid var(--glass-border); background:rgba(255,255,255,.05);
      color:var(--text-primary); font-family:'DM Sans',sans-serif; font-size:.875rem; outline:none; transition:all .2s;
    }
    .sinp:focus { border-color:var(--blue); background:rgba(58,141,222,.05); }
    .sinp::placeholder { color:var(--text-muted); }

    /* chips */
    .chips { display:flex; flex-wrap:wrap; gap:.48rem; margin-bottom:1.35rem; }
    .chip {
      padding:.3rem .82rem; border-radius:20px; border:1.5px solid var(--glass-border);
      background:var(--glass); color:var(--text-muted); font-size:.77rem; font-weight:500; cursor:pointer; transition:all .2s;
    }
    ...
    .tab {
      padding:.46rem 1.1rem; border-radius:9px; border:none;
      font-family:'DM Sans',sans-serif; font-size:.83rem; font-weight:600;
      cursor:pointer; transition:all .2s; color:var(--text-muted); background:transparent;
    }
    ...
    .evcard {
      background:rgba(10,18,60,.85); border:1px solid var(--glass-border);
      border-radius:18px; overflow:hidden; transition:all .25s; cursor:pointer; position:relative;
    }
...
const SEED_EVENTS = [
  ...
  { id:3, title:"Inter-College Cricket Tourney",  cat:"sports",   date:addDays(10), time:"8:00 AM",  venue:"Sports Complex", org:"Sports Dept.",    cap:200, desc:"Cheer for your college team in the biggest inter-college cricket tournament of the year!",                                       pub:true, regs:[] },
...
function Toasts({ toasts }) {
  ...
  return (
    <div className="toast-wrap">
      {toasts.map(t=>(
        <div key={t.id} className={`toast t-${t.type}`}>{ic[t.type]} {t.msg}</div>
      ))}
    </div>
  );
}
...
function CampusPanel() {
  const bldgs = [
    {l:"4%",w:"16%",h:"52%",n:12},{l:"21%",w:"20%",h:"70%",n:18},
    {l:"43%",w:"15%",h:"46%",n:9},{l:"60%",w:"22%",h:"63%",n:15},{l:"84%",w:"14%",h:"40%",n:6},
  ];
  return (
  ...
  return (
    <div className="auth-root">
      ...
      <div className="auth-right">
        {/* mobile logo */}
        <div className="mobile-logo fade-up">
          <div className="mob-logo-box">CE</div>
          <span className="mob-logo-text">Campus<em>Events</em></span>
        </div>

        <h1 className="auth-heading fade-up">Create account</h1>
        <p className="auth-sub fade-up">Register to access college events</p>

        {err && <div className="err-box fade-in">⚠️ {err}</div>}

        {/* name + role row */}
        <div className="form-row fade-up2">
          <div className="form-group">
            <label className="form-label">Full Name *</label>
            <input className={`form-input ${err&&!form.name.trim()?"err":""}`}
              placeholder="Rahul Verma" value={form.name}
        ...
        <div className="form-group fade-up3">
          ...
          <div className="pw-wrap">
            ...
            <button className="pw-eye" onClick={()=>setShowPw(v=>!v)}>
              {showPw?<EyeOff/>:<EyeOn/>}
            </button>
          </div>
        ...
        <button className="auth-btn fade-up4" onClick={submit} disabled={loading}>
          {loading
  ...
  return (
    <div className="auth-root">
      ...
      <div className="auth-right">
        ...
        <h1 className="auth-heading fade-up">Welcome back</h1>
        <p className="auth-sub fade-up">Sign in to your campus events account</p>

        {/* Google */}
        <button className="google-btn fade-up2" onClick={()=>toast.info("Google sign-in coming soon!")}>
          <div className="g-icon">G</div>
          Continue with Google
        </button>

        <div className="or-div fade-up2">
        ...
        {/* quick demo fill */}
        <div className="quick-btns fade-up3">
  ...
  <a href="#" onClick={(e) => { e.preventDefault(); onSwitch(); }}>
    Create
        ...
        <div className="demo-hint fade-up5">
          ...
          <p>New accounts you create via signup also work!</p>
        </div>
      </div>
    </div>
  );
...
function EventCard({ ev, user, onOpen, onReg }) {
  ...
  return (
    <div className="evcard fade-up" onClick={()=>onOpen(ev)}>
...
function StudentDash({ user, events, setEvents, toast }) {
  ...
  return (
    <>
      ...
      <div className="sec-hdr fade-up3">
        <h2 className="sec-title">
          {tab==="discover"?<>Upcoming <em>Events</em></>:tab==="past"?"Past Events":<>My <em>Registrations</em></>}
        </h2>
    ...
    <div className="mo" onClick={onClose}>
      <div className="mbox" style={{maxWidth:600}} onClick={e=>e.stopPropagation()}>
        ...
        <div className="ce-grid">
          ...
          <div className="form-group"><label className="form-label">Organizer *</label><input className="form-input" placeholder="e.g. CS Department" value={form.org} onChange={e=>set("org",e.target.value)}/></div>
          ...
          <div className="form-group"><label className="form-label">Time</label><input className="form-input" placeholder="10:00 AM" value={form.time} onChange={e=>set("time",e.target.value)}/></div>
          <div className="form-group"><label className="form-label">Venue *</label><input className="form-input" placeholder="e.g. Main Auditorium" value={form.venue} onChange={e=>set("venue",e.target.value)}/></div>
          <div className="form-group"><label className="form-label">Capacity</label><input type="number" className="form-input" min={10} max={2000} value={form.cap} onChange={e=>set("cap",Number(e.target.value))}/></div>
          <div className="form-group" style={{gridColumn:"1/-1"}}><label className="form-label">Description *</label><textarea className="form-input" rows={3} placeholder="Brief description…" value={form.desc} onChange={e=>set("desc",e.target.value)}/></div>
...
function AdminDash({ user, events, setEvents, users, toast }) {
  ...
  return (
    <>
      <div className="hero fade-up" style={{background:"linear-gradient(135deg,#1a0f40,#2a1460)"}}>
      ...
      <div className="stat-row fade-up2">
        {[["Total Events",events.length,"var(--gold)",`${upcoming} upcoming`],
          ...
          ["Active Students",studs,"#c084fc","registered ≥1 event"]].map(([l,v,c,s])=>(
      ...
      {tab==="events" && (
        <>
          ...
          <div className="tbl-wrap fade-up3">
            <table className="tbl">
                ...
                {filtered.map(e=>{
                  ...
                  return(
                    <tr key={e.id}>
                      ...
                      <td>
                        <button className="btn btn-ghost btn-sm" onClick={()=>setRegEv(e)}>
                          <UsrIco/> {e.regs.length}/{e.cap}
                        </button>
                      </td>
